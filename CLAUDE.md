...
- **控制器** `xlsx-viewer.js`（碰 DOM）：主題切換、i18n、拖拉 / 上傳、檔案清單、`renderWorkbook(wb)`（多 sheet → `.tabs` + `.sheet-panel`，**單一工作表隱藏 tab bar**；**自管切換**：click toggle `.active` + CSS 底線，**不用 `M.Tabs.init`**；tab flex 等分縮放 + 省略號 + `title`）、`?xlsx=` 深連結。切檔時 `clearOutput()` 再渲染。
...
- **side-tool**：`#setting-menu`（檔案清單）/ `#setting-download`（下載原始檔，只在開檔時顯示、臨時 `<a download>` + check 回饋、href 經 `encodePath`）/ `#setting-clear`（清空，hover 轉紅）/ `#setting-mode` / `#setting-lang`；〔正統〕flex `.side-tools`，**chrome 墊底**（§5.5 排列順序，2026-07-26 收斂）。**下載走側鍵、toolbar 不放操作鍵**（家族 §4.7）。
